...
def write_header(file, file3):
    ...
    file3.write('# This file was generated by _scripts/gen_python.py from the rethinkdb documentation in http://github.com/rethinkdb/docs\n')
    file3.write('# hash: '+commit+'\n\n\n')
    file3.write('import rethinkdb\n')

# Browse all the docs
def browse_files(base, result_file, result_file3):
    subdirlist = []
    ...
    for item in sorted(os.listdir(base)):
        if item[0] != '.' and item not in IGNORED_FILES:
            ...
            if os.path.isfile(full_path):
                add_doc(full_path, result_file, result_file3)
            else:
                subdirlist.append(full_path)

    for subdir in subdirlist:
        browse_files(subdir, result_file, result_file3)


# Add docs in result for one file
def add_doc(file_name, result_file, result_file3):
...
if __name__ == "__main__":
    ...
    result_file3 = codecs.open(script_path+"/docs3.py", "w", "utf-8")
    
    write_header(result_file, result_file3)

    browse_files(script_path+"/../api/python/", result_file, result_file3)
